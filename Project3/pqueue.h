...
private:
    ...
    //Overloaded clear function that wipes a PQueue object.
    void clear(Node* root);
    //Recursive merging function that combines two heaps.
    Node* merge(Node* lhs, Node* rhs);
    //Helper function that copies the rhs heap tree to lhs.
    Node* copyHeap(Node* root);
    //Overloaded printPatientQueue function to recursively print out the heap.
    void printPatientQueue(Node* root) const;
    //Helper function that calculates a node's npl
    void updateNPL(Node* node);
    //Helper function to determine whether swaps are req for leftist heaps.
    void checkNPLs(Node* node);
};
