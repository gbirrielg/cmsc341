...
class CarDB{
    ...
    ******************************************/
    ...
    //Overloaded insert function to handle standard insertions and collisions.
    bool insert(Car car, int i);
    //Helper function that inserts without checking integrity or car transfers.
    void insertNoCheck(Car car, int i);
    //Overloaded remove function to handle search and deletion of a car in current table.
    bool removeCurrent(Car car, int i);
    //Overloaded remove function to handle search and deletion of a car in old table.
    bool removeOld(Car car, int i);
    //Helper function that removes from old table without checking integrity or car transfers.
    void removeOldNoCheck(Car car, int i);
    //Helper function to check integrity of hash table and create new one if necessary.
    void checkIntegrity();
    //Helper function to transfer 25% of elements from old table to new table every call.
    void transferCars();
    //Overloaded getCar function to find and return a car in either tables.
    Car getCar(string model, int dealer, int i, Car* table) const;
    //Helper function to return the index of a searched for car, if it exists.
    int search(Car car, int i, Car* table);
    //Helper function to return the deletion ratio of the oldTable. Used for transferring.
    float deletedRatioOld() const;
};
#endif